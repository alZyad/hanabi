  ...
  const tutorialActions: TutorialAction[] = [
    {
      ...
      content: (
        <Trans i18nKey="tutorialActions.beginning.content">
          ...
          {card(IColor.YELLOW, 1, colorBlindMode)}
          and
          {card(IColor.BLUE, 1, colorBlindMode)}.<br />
          Let's tell him by hinting him 1s.
        </Trans>
      ),
      todo: (
        <Trans i18nKey="tutorialActions.beginning.todo">
          Tap Adam's game, select {vignette("number", 1, colorBlindMode)}, then click "Hint"
        </Trans>
      ),
    },
    {
      ...
      content: (
        <Trans i18nKey="tutorialActions.playFirstRed.content">
          ...
          <br />
          Jane also gave you a {vignette("color", IColor.RED, colorBlindMode)} hint on 2 cards. It might mean those
          cards are interesting to play right now.
          <br />
    ...
    },
    {
      ...
      content: (
        <Trans i18nKey="tutorialActions.playSecondRed.content">
          ...
          mistake.
          <br />
          Let's be optimistic and assume that our third card is {card(IColor.RED, 2, colorBlindMode)}
        </Trans>
      ),
    ...
    },
    {
      ...
      content: (
        <Trans i18nKey="tutorialActions.hint.content">
          ...
          {card(IColor.BLUE, 3, colorBlindMode)} and she plays it, Adam will understand he can play his{" "}
          {card(IColor.BLUE, 4, colorBlindMode)}
        </Trans>
      ),
      todo: (
        <Trans i18nKey="tutorialActions.hint.todo">
          Tap Jane's game and hint her {vignette("color", IColor.BLUE, colorBlindMode)}s
        </Trans>
      ),
    },
    {
      ...
      content: (
        <Trans i18nKey="tutorialActions.interpret.content">
          ...
          <br />
          We received a {vignette("color", IColor.YELLOW, colorBlindMode)} hint. Can you guess what it means?
        </Trans>
      ),
    ...
    },
    {
      ...
      content: (
        <Trans i18nKey="tutorialActions.save.content">
          😅 Adam just discarded {card(IColor.GREEN, 5, colorBlindMode)}. There's only one{" "}
          {vignette("number", 5, colorBlindMode)} for each color in the deck so it should never be discarded!
          <br />
          <br />
          It might have been predicted using the <span className="txt-blue">right-most discard</span> convention. Adam
          had info about his latest card, so he chose to discard {card(IColor.GREEN, 5, colorBlindMode)} instead.
          <br />
          <br />
      ...
      todo: (
        <Trans i18nKey="tutorialActions.save.todo">Tell Jane about her {card(IColor.YELLOW, 5, colorBlindMode)}</Trans>
      ),
    },
  ];
